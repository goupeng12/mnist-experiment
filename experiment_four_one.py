"""
本实验在experiment_four.py的基础上，做了如下改动：
@变更了损失函数
"""
import input_data
...
x2 = tf.nn.relu(W1x1b1) #激活函数			  60000x10的矩阵
...
x3 = tf.nn.relu(W2x2b2) #激活函数				  60000x784的矩阵
...
y = tf.nn.softmax(Wxb) #经softmax后，变为归一化的 60000x10 的概率矩阵
...
with tf.name_scope('loss'):
	loss = tf.reduce_mean(-tf.reduce_sum(y * tf.log(tf.clip_by_value(Wxb,1e-10,1.0)), reduction_indices = [1]))
...
train_writer = tf.summary.FileWriter('exp_four_one_train_logs',sess.graph) #将训练日志写入到logs文件夹下
