...
with tf.name_scope('loss'):
	loss = tf.reduce_mean(-tf.reduce_sum(y * tf.log(tf.clip_by_value(y_,1e-10,1.0)), reduction_indices = [1]))
...
train_writer = tf.summary.FileWriter('exp_four_two_train_logs',sess.graph) #将训练日志写入到logs文件夹下
